          ...
          <div className={`${isMobile ? 'flex flex-col gap-6' : 'max-w-4xl mx-auto space-y-8'}`}>
            {feedItems.map((item) => {
              if (item.type === 'flashcard') {
                return <FlashCard key={item.id} item={item} onShare={handleShare} />;
              } else if (item.type === 'text') {
                return <TextContentCard key={item.id} item={item} onShare={handleShare} />;
              } else if (item.type === 'mcq') {
                return <McqCard key={item.id} item={item} onShare={handleShare} />;
              }
              return null;
            })}
          </div>
