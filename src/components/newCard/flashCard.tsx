...
function FlashcardReel({ question, answer, subtitle, theme, tag, author, time, upvotes, comments, onCommentToggle }: any) {
  const [flipped, setFlipped] = useState(false);
...
function FlashcardExplore({ question, author, upvotes, bg = "bg-[#fef3ea]" }: any) {
  return (
...
function FlashcardProfile({ question }: { question: string }) {
  return (
