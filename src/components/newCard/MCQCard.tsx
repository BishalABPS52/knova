...
export function MCQCard(props: any) {
...
function MCQFeed({ author, time, question, options, correctIndex, explanation, upvotes, downvotes, authorInitial, authorBg, authorImg }: any) {
  ...
  return (
    <div className="glass-card rounded-2xl overflow-hidden hover-lift transition-all duration-300">
      ...
      <FeedActions upvotes={upvotes} downvotes={downvotes} comments={0} />
    </div>
  );
}

function MCQReel({ question, options, correctIndex, explanation, tags, author, time, upvotes, comments, onCommentToggle }: any) {
  const [selected, setSelected] = useState<number | null>(null);
...
function MCQExplore({ tag, question, options, author, upvotes, correctIndex = -1 }: any) {
  return (
...
function MCQProfile({ tag, question, options, highlightIndex = -1 }: any) {
  return (
